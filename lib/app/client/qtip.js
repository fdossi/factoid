...
$(function(){
  $.fn.qtip.defaults = $.extend(true, {}, $.fn.qtip.defaults, {
    ...
    position: {
      container: $('#qtips'), // by default so we don't get them destroyed if the body gets replaced by derby's templating
      my: 'top center',
      at: 'bottom center',
      effect: false,
      viewport: true,
...
$.helperQtip = function( options, qtipOptions ){
  ...
  $('#qtip-helpers').append( $helper );
  $helper.qtip( qtipOptions );

  id = 'qtip-' + $helper.qtip('api').id;
};

// TODO refactor so that this just shows instead of creating as well
