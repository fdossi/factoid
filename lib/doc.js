...
// NB: assumes that `_doc` is set to the doc path in the model properly
...
module.exports = function( model, srvs ){
  ...
  // subscribe to the document model and do any necessary model/doc setup
  ...
  function subscribe(docId, next){
    var docPath = 'documents.' + docId;

    return model.subscribe(docPath, 'entities', 'organisms', function(err, doc, entities){
      ...
      for( var i = 0; i < orgs.length; i++ ){
        ...
        model.setNull('organisms.' + i + '.documents.' + docId + '.enabled', true);
      }
  ...
  */
  ...
  function togglePopover( model, showVar, aniVar, next ){
    var shown = model.get( showVar ) ? true : false;
  ...
  function hidePopover( model, showVar, aniVar, next ){
    popoversOpen = false;
  ...
  function showPopover( model, showVar, aniVar, next ){
    popoversOpen = true;
  ...
  return doc = {
    ...
    closeAllPopovers: function(){
      ...
      for( var i = 0; i < ents.length; i++ ){
        ...
        if( ent._showAddParticipants === true ){ // must do selectively like this to prevent many model updates
          model.set('_entities.' + i + '._showAddParticipants', false);
        }

        if( ent._showEdit === true ){
          model.set('_entities.' + i + '._showEdit', false);
        }

        if( ent._showAnnotations === true ){
          model.set('_entities.' + i + '._showAnnotations', false);
        }
    ...
    toggleAnnotations: function( entId, next ){
      togglePopover( model.at('entities').at(entId), '_showAnnotations', '_showAnnotationsAnimated', function(shown){
    ...
    showTextmining: function( next ){
      showPopover( model, '_showTextmining', '_showTextminingAnimated', next );
    },

    hideTextmining: function( next ){
      hidePopover( model, '_showTextmining', '_showTextminingAnimated', next );
    },
    ...
    goToStage: function( stage ){
      var fn = arguments[0];
      if( isFunction(fn) ){
        model.on('set', '_doc.stage', function(){
          var stage = model.get('_doc.stage');
