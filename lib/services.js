...
module.exports = function(_, http, config, request, form, nodemailer){
  ...
  function addDoc( textDoc, next ){
    request( {
      ...
      var docId = body;
      console.log('--\nAdding new text doc to textmining\n--');
      console.dir(textDoc);
      console.log('--\nNew doc has ID `' + docId + '`\n--');
      next(error, docId);
    });
  }
  ...
  function getAbner(docId, next){
    request( {
      ...
      console.log('--\nGetting abner for `' + docId + '`\n--');
      console.dir(retAbner);
      next(error, retAbner);
    });
  }
  ...
  function getAssociatedEntitiesFromQuery( query, next ){
    ...
    //   pres.on('data', function (chunk) { console.log('got back response from uniprot', chunk);
    //     var textResp = chunk;
    //     var lines = textResp.split('\n');
    ...
    // console.log('sending request to uniprot');
